...
class Cluster():
    def __init__(self, name, data, linkage="single"):
        # linkage: the distance between two clusters(default=single)
        self.linkage = linkage
        self.name = name
        # expand the shape of x from (?,) to (1, ?)
        self.data = data[np.newaxis, :] if len(data.shape) == 1 else data
        assert len(self.data.shape) == 2

    def __sub__(a, b):
        """calculate two samples' L2 distance"""
        distance = np.Inf
        if a.linkage == "single":
            for i in a.data:
                for j in b.data:
                    ...
                    if dij < distance:
                        distance = dij
        # TODO: complete, average, award linkage
        return distance
...
class ClusterKmeans(Clustering):
    def fit(self, x):
        n_samples = x.shape[0]
        # random set k center
        centroids_idx = np.random.randint(x.shape[0], size=self.k)
        centroids = x[centroids_idx]
        # remove the duplicate centroids in x
        x = np.delete(x, [centroids_idx], axis=0)
        # convergence judgement
        n_iter = 0
        # iteration for maxiter times
        while n_iter <= self.maxiter:
            ...
            # print(gs[0].data.shape[0], gs[1].data.shape[0])
            n_iter += 1
        self.gs = gs
